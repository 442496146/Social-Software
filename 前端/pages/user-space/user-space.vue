		...
		methods: {
			...
			async getUserInfo(userid){
				...
				if (userid == this.User.userinfo.id) {
					...
					// 非本人，获取用户信息
					let [err,res] = await this.$http.get('/getuserinfo/'+userid,{},{
						token:true
					})
					// 错误处理
				...
				let regtime = info.create_time ? Time.gettime.dateFormat(new Date(info.create_time),'{Y}-{MM}-{DD}') : "未知";
				this.userinfo = {
					isme:isme,
					bgimg:1,
